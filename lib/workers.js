...
workers.performCheck = function (checkData) {
  ...
  const req = _moduleToUse.request(requestDetails, function (res) {
    ...
    console.log('En el unico log stored, se guardó 200 y debia ser 0. revisar ', status);
...
workers.processCheckOutcome = async function (checkData, checkOutcome) {
  ...
  const state =
    Object.keys(checkOutcome.error).length === 0 &&
  ...
  if (alertWarranted) {
    ...
    //END OF THIS LOGIC
    console.log('checkOutCome.error ', checkOutcome.error);
    const logItem = new logsModel({
      ...
      lastChecked: checkData.lastChecked,
